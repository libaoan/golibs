...
完整的演示例子源码见 [src/std01/io/io.go](/src/std01/io/io.go)
...
程序从标准输入接收一个字节（ASCII 字符），调用 buffer 的 WriteByte 将该字节写入 buffer 中，之后通过 ReadByte 读取该字节。完整的代码见：[src/std01/io/io.go](/src/std01/io/io.go)
